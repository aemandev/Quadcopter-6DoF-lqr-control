...
// Generated 14-Nov-2021 12:55:45
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"lqr_pos_control\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(lqr_pos_control_M, (NULL));
        lqr_pos_control_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(lqr_pos_control_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(lqr_pos_control_M)
                == (NULL)) || rtmGetStopRequested(lqr_pos_control_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(lqr_pos_control_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
